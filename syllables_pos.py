...
for count, _id in enumerate(docids):
    ...
    #if it's in gutenberg_full_text or other
    if sources[count] == 'lavin':
    #if sources[count] == 'gutenberg' or sources[count] == 'lavin':
        print(_id)
        full_pos_dict = {}
        full_syllable_dict = {}
        try:
        ...
        for y in c:
            ...
            try:
                ...
                sylls = nsyl(y[0])
                #add to dictionary
                full_syllable_dict[y[0]] = [int(y[1]), sylls]
            except:
                pass
        ...
        #add results to dictionary by id
        full_feature_dicts_syllables[_id] = full_syllable_dict
        full_feature_dicts_pos[_id]= full_pos_dict
        ...
        try:
            ...
            store_results(result_row)
            print("ID %s worked" % str(_id))

        except:
            print("ID %s didn't work" % str(_id))

pickle.dump(full_feature_dicts_pos, open( "pickled_data/full_feature_dicts_pos.p", "wb" ))
pickle.dump(full_feature_dicts_syllables, open( "pickled_data/full_feature_dicts_syllables.p", "wb" ))
